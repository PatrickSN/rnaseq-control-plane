...
def auth_login(email: str = typer.Option(...), password: str = typer.Option(..., hide_input=True)) -> None:
    token = request("POST", "/api/auth/login", json={"email": email, "password": password})
    save_config({"token": token["access_token"]})
    console.print("Authenticated.")


@pipelines_app.command("list")
def pipelines_list(json_output: bool = typer.Option(False, "--json")) -> None:
...
def runs_create(
    pipeline: str = typer.Option(..., help="Pipeline slug or UUID."),
    params_file: Path | None = typer.Option(None, "--params-file", exists=True, readable=True),
    ...
    run = request("POST", "/api/runs", json=payload)
    if json_output:
        emit_json(run)
    else:
...
def runs_list(json_output: bool = typer.Option(False, "--json")) -> None:
    ...
    for run in runs:
        table.add_row(run["id"], run["name"] or "-", run["status"], run["profile"], run["session_id"] or "-")
    console.print(table)


@runs_app.command("show")
def runs_show(run_id: str, json_output: bool = typer.Option(False, "--json")) -> None:
